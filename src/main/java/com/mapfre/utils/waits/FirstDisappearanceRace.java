 ...
 * un `errorRoot` o un `successRoot` (cuando desaparece/se oculta).
 *
 * Estrategia (resumen):
 * - Opcionalmente ejecuta una acción que dispara el cambio (p. ej. un click).
 * - Hace polling ligero leyendo si cada locator es visible o está oculto.
 * - Cuando un locator se mantiene en su estado objetivo durante `quietMs` milisegundos se considera "estable".
 * - Si ambos se estabilizan, se desempata el que cambió primero (por timestamp).
 * - Devuelve un `Result` con Outcome {ERROR, SUCCESS, TIMEOUT} y, para ERROR, intenta leer
 *   el texto del mensaje de error.
 */
public final class FirstDisappearanceRace {

    private FirstDisappearanceRace() {}
    ...
    /**
     * Wait until either errorRoot becomes visible or successRoot becomes hidden first (and stays in that state for quietMs).
     *
     * Parámetros clave:
     ...
     * - errorRoot / successRoot: locators que representan estado de error y éxito (oculto) respectivamente.
     ...
     * - quietMs: ventana de estabilidad: el locator debe mantener su estado (visible para error,
     *   oculto para éxito) de forma continua durante al menos quietMs para considerarse "estable".
     *   Usar 0 para aceptar cambio instantáneo.
    ...
    public static Result waitForFirst(
        ...
        // Timestamps (en nanos) indicando cuándo cada locator alcanzó su estado objetivo de forma
        // continua. -1 significa "no alcanzado" actualmente.
        // Para errorRoot: objetivo es estar visible.
        // Para successRoot: objetivo es estar oculto.
        long errorSeenAt = -1;
        ...
        while (System.nanoTime() < deadline) {
            ...
            // Track continuous-state start times
            // Para errorRoot: objetivo es visible (ERROR)
            // Para successRoot: objetivo es oculto (SUCCESS)
            // Si el estado objetivo se mantiene, guardamos el tiempo de inicio;
            // si cambia de estado, reiniciamos el timestamp a -1.
            // Comentario: esto permite detectar que un elemento mantiene su estado objetivo de forma "continua"
            // durante la ventana de quietMs. Si cambia y vuelve a cambiar rápidamente no se considera estable.
            if (errVisible) {
            ...
            // Determinar si cada uno es "estable": en su estado objetivo y en ese estado durante al menos quietMs.
            // Nota: si quietMs <= 0, aceptamos el cambio instantáneo (sin ventana).
            ...
            // Si ambos son estables, desempatar por quién alcanzó su estado objetivo primero.
            // Si los timestamps son iguales, la condición errorSeenAt <= successSeenAt favorecerá ERROR.
            // Comentario: el desempate por timestamp cubre casos donde ambos elementos cambian casi al
            // mismo tiempo; se asume que el que cambió primero es el estado "válido".
